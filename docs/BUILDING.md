...
- **arduino-cli** for the Cardputer Adv RNode component build
- **Git**
...
## Recommended Cardputer Adv Build

The default Cardputer Adv build is the dual-mode family image. It contains:

- launcher in `ota_0`
- Ratcom in `ota_1`
- RNode in `ota_2`

The Cardputer Adv RNode firmware source is vendored in this repo:

```text
vendor/rnode_firmware/
```

Build the full image and release packages:
...
```text
dist/ratcom-cardputer-adv-full.bin
dist/ratcom-cardputer-adv-full.zip
dist/ratcom-cardputer-adv-ratcom-only.bin
dist/ratcom-cardputer-adv-ratcom-only.zip
dist/ratcom-cardputer-adv-rnode-only.bin
dist/ratcom-cardputer-adv-rnode-only.zip
```
...
files package the same merged image with a web-flasher manifest.

> **Upgrade warning:** the dual-mode image uses a different partition table than
> older standalone Ratcom builds. Treat it as a full firmware upgrade and expect
> existing flash-backed Ratcom data/config to be reset. SD-card data under
> `/ratcom/` may still be present, but migration is not guaranteed or assisted.

## Standalone Ratcom Build
...
`partitions/rs_cardputer_adv_8mb_dual.csv` — 8MB flash layout for the dual-mode
family image:

| Name | Type | Offset | Size | Purpose |
|------|------|--------|------|---------|
| nvs | data/nvs | 0x9000 | 20 KB | Shared NVS |
| otadata | data/ota | 0xE000 | 8 KB | Boot slot selection |
| launcher | app/ota_0 | 0x10000 | 1 MB | Ratspeak mode launcher |
| ratcom | app/ota_1 | 0x110000 | 0x260000 | Standalone Ratcom mode |
| rnode | app/ota_2 | 0x370000 | 0x260000 | Host-controlled RNode mode |
| littlefs | data/spiffs | 0x5D0000 | 0x220000 | Ratcom storage |
| coredump | data/coredump | 0x7F0000 | 64 KB | ESP-IDF core dump |
...
- **Package**: Triggers on push to `main` and PRs. Runs `make package` and uploads all files in `dist/`.
- **Release**: Triggers on `v*` tags. Runs `make package` and attaches the full plus standalone `.bin`/`.zip` artifacts to the GitHub Release.
