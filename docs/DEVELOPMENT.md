...
Ratcom is a **dual-mode Cardputer Adv firmware family** built for the
M5Stack Cardputer Adv with Cap LoRa-1262 radio. The default firmware image
contains a launcher, the standalone Ratcom messenger, and a host-controlled
RNode mode.

This guide mostly documents the standalone Ratcom app. Ratcom mode is not a
KISS RNode; it runs its own Reticulum stack (microReticulum) directly on the
device. RNode mode is provided by the vendored firmware under
`vendor/rnode_firmware/`.

Key characteristics:
- Launcher-based mode switching between Ratcom and RNode
...
```
launcher/                       Mode selector application
vendor/rnode_firmware/          Cardputer Adv RNode mode source
partitions/                     Dual-mode ESP32-S3 partition table
tools/                          Image merge, fit-check, and packaging helpers
src/
...
Primary storage for all persistent data. In the dual-mode image this is the
LittleFS partition at offset `0x5D0000`.
